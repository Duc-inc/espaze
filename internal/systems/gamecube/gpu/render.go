...
// DrawTriangle rasterizes one flat/Gouraud-shaded, Z-tested triangle.
func (f *Framebuffer) DrawTriangle(t Triangle) {
	...
	for py := minY; py < maxY; py++ {
		for px := minX; px < maxX; px++ {
			...
			bl := byte(b0*float64(t.V1.B) + b1*float64(t.V2.B) + b2*float64(t.V0.B))
			f.frame.SetPixel(int(px), int(py), r, g, bl, 0xFF)
		}
	}
}
