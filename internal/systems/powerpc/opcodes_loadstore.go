...
func init() {
	setPrimary(32, func(c *CPU, instr uint32) int { // lwz
		c.regs.GPR[fieldRD(instr)] = c.read32(c.effectiveAddr(instr))
		return 3
	})
	setPrimary(33, func(c *CPU, instr uint32) int { // lwzu
		addr := c.effectiveAddr(instr)
		c.regs.GPR[fieldRD(instr)] = c.read32(addr)
		c.regs.GPR[fieldRA(instr)] = addr
		return 3
	})
	setPrimary(34, func(c *CPU, instr uint32) int { // lbz
		c.regs.GPR[fieldRD(instr)] = uint32(c.read8(c.effectiveAddr(instr)))
		return 3
	})
	setPrimary(35, func(c *CPU, instr uint32) int { // lbzu
		addr := c.effectiveAddr(instr)
		c.regs.GPR[fieldRD(instr)] = uint32(c.read8(addr))
		c.regs.GPR[fieldRA(instr)] = addr
		return 3
	})
	setPrimary(40, func(c *CPU, instr uint32) int { // lhz
		c.regs.GPR[fieldRD(instr)] = uint32(c.read16(c.effectiveAddr(instr)))
		return 3
	})
	setPrimary(41, func(c *CPU, instr uint32) int { // lhzu
		addr := c.effectiveAddr(instr)
		c.regs.GPR[fieldRD(instr)] = uint32(c.read16(addr))
		c.regs.GPR[fieldRA(instr)] = addr
		return 3
	})

	setPrimary(36, func(c *CPU, instr uint32) int { // stw
		c.write32(c.effectiveAddr(instr), c.regs.GPR[fieldRD(instr)])
		return 3
	})
	setPrimary(37, func(c *CPU, instr uint32) int { // stwu
		addr := c.effectiveAddr(instr)
		c.write32(addr, c.regs.GPR[fieldRD(instr)])
		c.regs.GPR[fieldRA(instr)] = addr
		return 3
	})
	setPrimary(38, func(c *CPU, instr uint32) int { // stb
		c.write8(c.effectiveAddr(instr), byte(c.regs.GPR[fieldRD(instr)]))
		return 3
	})
	setPrimary(39, func(c *CPU, instr uint32) int { // stbu
		addr := c.effectiveAddr(instr)
		c.write8(addr, byte(c.regs.GPR[fieldRD(instr)]))
		c.regs.GPR[fieldRA(instr)] = addr
		return 3
	})
	setPrimary(44, func(c *CPU, instr uint32) int { // sth
		c.write16(c.effectiveAddr(instr), uint16(c.regs.GPR[fieldRD(instr)]))
		return 3
	})
	setPrimary(45, func(c *CPU, instr uint32) int { // sthu
		addr := c.effectiveAddr(instr)
		c.write16(addr, uint16(c.regs.GPR[fieldRD(instr)]))
		c.regs.GPR[fieldRA(instr)] = addr
		return 3
	})
	setPrimary(42, func(c *CPU, instr uint32) int { // lha (sign-extended halfword)
		c.regs.GPR[fieldRD(instr)] = uint32(int32(int16(c.read16(c.effectiveAddr(instr)))))
		return 3
	})
	setPrimary(43, func(c *CPU, instr uint32) int { // lhau
		addr := c.effectiveAddr(instr)
		c.regs.GPR[fieldRD(instr)] = uint32(int32(int16(c.read16(addr))))
		c.regs.GPR[fieldRA(instr)] = addr
		return 3
	})

	setExt31(23, func(c *CPU, instr uint32) int { // lwzx
		c.regs.GPR[fieldRD(instr)] = c.read32(c.effectiveAddrIndexed(instr))
		return 3
	})
	setExt31(87, func(c *CPU, instr uint32) int { // lbzx
		c.regs.GPR[fieldRD(instr)] = uint32(c.read8(c.effectiveAddrIndexed(instr)))
		return 3
	})
	setExt31(279, func(c *CPU, instr uint32) int { // lhzx
		c.regs.GPR[fieldRD(instr)] = uint32(c.read16(c.effectiveAddrIndexed(instr)))
		return 3
	})
	setExt31(343, func(c *CPU, instr uint32) int { // lhax
		c.regs.GPR[fieldRD(instr)] = uint32(int32(int16(c.read16(c.effectiveAddrIndexed(instr)))))
		return 3
	})
	setExt31(151, func(c *CPU, instr uint32) int { // stwx
		c.write32(c.effectiveAddrIndexed(instr), c.regs.GPR[fieldRD(instr)])
		return 3
	})
	setExt31(215, func(c *CPU, instr uint32) int { // stbx
		c.write8(c.effectiveAddrIndexed(instr), byte(c.regs.GPR[fieldRD(instr)]))
		return 3
	})
	setExt31(407, func(c *CPU, instr uint32) int { // sthx
		c.write16(c.effectiveAddrIndexed(instr), uint16(c.regs.GPR[fieldRD(instr)]))
		return 3
	})
