...
func (c *CPU) Reset() {
	...
	// NR52 (sound power) must land before any other sound register: the
	// APU ignores writes to the rest of its registers while powered off,
	// and map iteration order isn't guaranteed, so this can't just be
	// another entry in postBootIO below.
	c.bus.Write(0xFF26, 0xF1)
