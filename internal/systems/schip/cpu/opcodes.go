...
// execute decodes and runs a single 16-bit opcode: every classic CHIP-8
// instruction plus the Super-CHIP additions (scrolling, hi-res mode,
// 16x16 sprites, the big font and RPL user-flag storage).
...
func (c *CPU) execute0(opcode uint16, n byte) error {
	if opcode&0xFFF0 == 0x00C0 {
		c.disp.ScrollDown(int(n))
		return nil
	}
	switch opcode {
	case 0x00E0:
	...
	case 0x00FD:
		c.halted = true
	case 0x00FE:
		c.disp.SetExtended(false)
	case 0x00FF:
		c.disp.SetExtended(true)
	}
	return nil
}
...
func (c *CPU) execute8(x, y, n byte) {
	...
	case 0x6:
		// SCHIP quirk: shifts VX in place and ignores VY entirely.
		c.regs.V[0xF] = vx & 0x1
		c.regs.V[x] = vx >> 1
	case 0x7:
		c.regs.V[x] = vy - vx
...
func (c *CPU) executeF(x byte, kk byte) error {
	...
	case 0x30:
		c.regs.I = BigFontStart + uint16(c.regs.V[x])*10
	case 0x33:
		v := c.regs.V[x]
...
// drawSprite handles both the classic 8-wide sprite (n = row count) and
// the Super-CHIP 16x16 sprite (n == 0, 32 bytes: 2 per row).
func (c *CPU) drawSprite(x, y, n byte) {
	...
	sprite := make([]byte, byteCount)
	for i := 0; i < byteCount; i++ {
		sprite[i] = c.mem.Read(c.regs.I + uint16(i))
	}

	collision := c.disp.DrawSprite(int(c.regs.V[x]), int(c.regs.V[y]), sprite, wide)
